...
e1000_transmit(struct mbuf *m)
{
  ...
  // free the desc's mbufs (if have one)
  if (tx_mbufs[index] != 0) 
    mbuffree(tx_mbufs[index]);
  tx_mbufs[index] = m;    // load mbuf(param)

  // fill the descriptor according to mbuf
  tx_ring[index].addr = (uint64)m->head;
  tx_ring[index].length = (uint16)m->len;
  tx_ring[index].cmd = E1000_RXD_STAT_DD | E1000_RXD_STAT_EOP;
